...
pub fn format_chip(ui: &mut Ui, label: &str, selected: bool) -> Response {
    ...
    if ui.is_rect_visible(rect) {
        let (bg, fg, stroke) = if selected {
            (p.accent_dim, p.accent_bright, Stroke::new(1.5, p.accent))
        } else if response.hovered() {
            (p.surface_high, p.text_primary, Stroke::new(1.0, p.base_light))
        } else {
            (p.surface_raised, p.text_secondary, Stroke::new(1.0, p.base))
        };
...
pub fn arrow_divider(ui: &mut Ui) {
    ...
    if ui.is_rect_visible(rect) {
        let c = rect.center();
        ui.painter().arrow(egui::pos2(c.x - 16.0, c.y), Vec2::new(32.0, 0.0), Stroke::new(2.0, p.accent));
    }
    ui.add_space(4.0);
}